...
Buffer::Buffer(Context& ctx, VkDeviceSize size, Usage usage)
    : ctx_(&ctx), size_(size) {
  if (size == 0) throw Error("vulkore: zero-sized buffer allocation");
...
void Buffer::upload_bytes(const void* src, VkDeviceSize size, VkDeviceSize dst_offset) {
  if (size == 0) return;
  if (!valid()) throw Error("vulkore: upload to empty buffer");
  if (dst_offset + size > size_) throw Error("vulkore: upload out of bounds");
...
void Buffer::download_bytes(void* dst, VkDeviceSize size, VkDeviceSize src_offset) {
  if (size == 0) return;
  if (!valid()) throw Error("vulkore: download from empty buffer");
  if (src_offset + size > size_) throw Error("vulkore: download out of bounds");
