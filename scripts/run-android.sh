#!/usr/bin/env bash
# =============================================================================
# run-android.sh — build (optional) and run the Xpose GoogleTest suite on a
...
#      to /data/local/tmp/xpose on the device.
#   4. Runs xpose_tests with XPOSE_KERNEL_DIR pointing at the pushed kernels,
...
DEVICE_DIR="/data/local/tmp/xpose"          # on-device temp dir
...
Build (optionally) and run the Xpose GoogleTest suite on a connected Android
device over adb.
...
Options:
  -s <serial>              Target a specific adb device (see 'adb devices').
      ...
      --device-filter <substr>
                           Exported as XPOSE_DEVICE on-device: substring-match
...
BIN="${BUILD_DIR}/tests/xpose_tests"
...
"${ADB[@]}" push "$BIN" "${DEVICE_DIR}/xpose_tests" >/dev/null \
  || die "failed to push test binary"
"${ADB[@]}" shell "chmod 755 '${DEVICE_DIR}/xpose_tests'" \
...
# =============================================================================
# 5. Run the suite (XPOSE_KERNEL_DIR -> pushed kernels; propagate exit code).
# =============================================================================
remote_cmd="XPOSE_KERNEL_DIR='${DEVICE_DIR}/kernels'"
[[ -n "$DEVICE_FILTER" ]] && remote_cmd+=" XPOSE_DEVICE='${DEVICE_FILTER}'"
remote_cmd+=" '${DEVICE_DIR}/xpose_tests'"
