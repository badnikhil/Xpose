...
// VULKORE_DEVICE unset so device enumeration can never throw on a value that
...
const std::vector<vulkore::DeviceInfo>& visible_devices() {
  static const std::vector<vulkore::DeviceInfo> devices = [] {
...
TEST(ContextTest, EnvVarSelectsLlvmpipe) {
  // Constructing a Context with an unmatched VULKORE_DEVICE THROWS
...
TEST(ContextTest, TwoContextsOnDifferentDevicesCoexist) {
  ...
  // env forces VULKORE_DEVICE=llvmpipe for b, which throws without one) — so skip
  ...
  ScopedEnv env("VULKORE_DEVICE", nullptr);
  vulkore::Context a;  // default policy -> a hardware GPU when present
...
TEST(ContextTest, ErrorMessagesAreDescriptive) {
  // Unmatched VULKORE_DEVICE: message says "available:" and lists real device
  // name(s) so the user can see what to pick (context.cpp:186-189).
  {
    ScopedEnv env("VULKORE_DEVICE", "definitely-not-a-real-gpu-9000");
    try {
      vulkore::Context ctx;
      FAIL() << "expected Context to throw on unmatched VULKORE_DEVICE";
