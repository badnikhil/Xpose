...
TEST_F(LaunchTest, CrossKernelRawChain) {
  ...
  vulkore::Kernel saxpy = load("saxpy");
  vulkore::Kernel scale = load("scale_inplace");
  ...
  vulkore::Fence f1 = vulkore::launch(saxpy, {n}, yb, xb, a, n);   // yb = a*x + y0
  vulkore::Fence f2 = vulkore::launch(scale, {n}, yb, factor, n);  // yb *= factor
  EXPECT_TRUE(f2.wait());
...
TEST_F(LaunchTest, InterleavedDifferentKernels) {
  ...
  vulkore::Kernel scale = load("scale_inplace");  // 1 buffer + 2 POD, 8B push
  vulkore::Kernel add = load("vec_add");          // 3 buffers + 1 POD, 4B push
  const uint32_t n = 256;
  const int K = 50;  // 2*K = 100 launches in flight

  vulkore::Buffer sf = ctx_->alloc<float>(n);
  ...
  for (int it = 0; it < K; ++it) {
    ...
    running *= factor;
    fences.push_back(vulkore::launch(scale, {n}, sf, factor, n));
    fences.push_back(vulkore::launch(add, {n}, ob, ab, bb, n));  // idempotent
  }
...
TEST_F(LaunchTest, GridTooLargeThrows) {
  ...
  vulkore::Buffer xb = ctx_->alloc<float>(16);
  vulkore::Buffer yb = ctx_->alloc<float>(16);
  try {
    vulkore::launch(k, {grid_x}, yb, xb, 1.0f, 16u);
...
TEST_F(LaunchTest, GridTooLargeThrowsAllAxes) {
  ...
  for (int axis = 0; axis < 3; ++axis) {
    ...
    try {
      vulkore::launch(k, grid, yb, xb, 1.0f, 16u);
...
TEST_F(LaunchTest, ZeroGridYZThrows) {
  ...
  vulkore::Buffer yb = ctx_->alloc<float>(16);
  {
    ...
    try {
      vulkore::launch(k, {16, 0}, yb, xb, 1.0f, 16u);
  ...
  }
  {
    ...
    try {
      vulkore::launch(k, {16, 1, 0}, yb, xb, 1.0f, 16u);
