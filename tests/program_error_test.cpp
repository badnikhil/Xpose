...
// Constructs a Program from `bytes` expecting an vulkore::Error whose message
// contains `needle`; fails the test if no throw or the wrong message.
void expect_program_error(vulkore::Context& ctx, const std::vector<uint8_t>& bytes,
                          const std::string& needle) {
  try {
    vulkore::Program prog(ctx, bytes);
    ADD_FAILURE() << "expected vulkore::Error containing \"" << needle << "\"";
...
std::unique_ptr<vulkore::Context> ProgramErrorTest::ctx_;
...
TEST_F(ProgramErrorTest, EmptyOrTruncatedFile) {
  {
    ...
    const std::string p = write_temp("vulkore_prog_empty.spv", {});
    try {
      vulkore::Program::from_file(*ctx_, p);
  ...
  }
  {
    ...
    const std::string p =
        write_temp("vulkore_prog_odd.spv", std::vector<uint8_t>(22, 0xABu));
    try {
      vulkore::Program::from_file(*ctx_, p);
...
TEST_F(ProgramErrorTest, UnsupportedReflectionInstThrows) {
  ...
  for (const Case& c : cases) {
    SCOPED_TRACE(std::string("reflection inst ") + std::to_string(c.number) +
                 " -> " + c.needle);
    try {
      vulkore::Program prog(*ctx_, module_with_unsupported(c.number));
