...
std::unique_ptr<xpose::Context> ProgramTest::ctx_;
...
TEST_F(ProgramTest, SaxpyReflection) {
  xpose::Program prog =
      xpose::Program::from_file(*ctx_, kernel_path("saxpy"));
  EXPECT_EQ(prog.kernel_names(), std::vector<std::string>{"saxpy"});

  xpose::Kernel k = prog.kernel("saxpy");
  ASSERT_TRUE(k.valid());
...
TEST_F(ProgramTest, VecAddReflection) {
  xpose::Program prog =
      xpose::Program::from_file(*ctx_, kernel_path("vec_add"));
  xpose::Kernel k = prog.kernel("vec_add");
  const xpose::KernelInfo& info = k.info();
  ASSERT_EQ(info.args.size(), 4u);
...
TEST_F(ProgramTest, ScaleInplaceReflection) {
  xpose::Program prog =
      xpose::Program::from_file(*ctx_, kernel_path("scale_inplace"));
  xpose::Kernel k = prog.kernel("scale_inplace");
  const xpose::KernelInfo& info = k.info();
...
TEST_F(ProgramTest, CompareFpReflection) {
  xpose::Program prog =
      xpose::Program::from_file(*ctx_, kernel_path("compare_fp"));
  xpose::Kernel k = prog.kernel("compare_fp");
  const xpose::KernelInfo& info = k.info();
...
TEST_F(ProgramTest, KernelHandleIsCached) {
  xpose::Program prog =
      xpose::Program::from_file(*ctx_, kernel_path("saxpy"));
  xpose::Kernel k1 = prog.kernel("saxpy");
  xpose::Kernel k2 = prog.kernel("saxpy");
...
TEST_F(ProgramTest, MissingFileThrows) {
  EXPECT_THROW(xpose::Program::from_file(*ctx_, kernel_path("no_such")),
               xpose::Error);
}

TEST_F(ProgramTest, GarbageBytesThrow) {
  const std::vector<uint8_t> garbage(64, 0xAB);
  EXPECT_THROW(xpose::Program(*ctx_, garbage), xpose::Error);

  const std::vector<uint8_t> tiny = {0x03, 0x02, 0x23, 0x07};  // just magic
  EXPECT_THROW(xpose::Program(*ctx_, tiny), xpose::Error);
}
...
TEST_F(ProgramTest, EmptyKernelHandleThrows) {
  ...
  EXPECT_THROW(empty.info(), xpose::Error);
  EXPECT_THROW(xpose::launch(empty, {1}), xpose::Error);
}
