...
// xpose::Error it throws, and vk_result_name(). These touch no Vulkan device
...
// XP_CHECK on a non-success VkResult throws xpose::Error carrying the real
...
TEST(CheckTest, VkResultNameMapsKnownAndUnknown) {
  EXPECT_EQ(xpose::vk_result_name(VK_SUCCESS), "VK_SUCCESS");
  EXPECT_EQ(xpose::vk_result_name(VK_NOT_READY), "VK_NOT_READY");
  EXPECT_EQ(xpose::vk_result_name(VK_TIMEOUT), "VK_TIMEOUT");
  EXPECT_EQ(xpose::vk_result_name(VK_ERROR_DEVICE_LOST), "VK_ERROR_DEVICE_LOST");
  EXPECT_EQ(xpose::vk_result_name(VK_ERROR_OUT_OF_DEVICE_MEMORY),
            "VK_ERROR_OUT_OF_DEVICE_MEMORY");
  EXPECT_EQ(xpose::vk_result_name(VK_ERROR_UNKNOWN), "VK_ERROR_UNKNOWN");

  // Unmapped values -> default branch (context.cpp:40). VkResult is signed, so
  // exercise both a positive and a negative unmapped code.
  EXPECT_EQ(xpose::vk_result_name(static_cast<VkResult>(123456)),
            "VkResult(123456)");
  EXPECT_EQ(xpose::vk_result_name(static_cast<VkResult>(-99)), "VkResult(-99)");
}
