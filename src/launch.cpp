...
uint32_t group_count(const std::string& kernel, char axis, uint32_t global,
  ...
  if (global == 0) {
    throw Error("xpose::launch('" + kernel + "'): grid." + axis +
...
Fence launch_impl(Kernel& kernel, Grid grid, const LaunchArg* args,
                  size_t count) {
  if (!kernel.valid()) {
    throw Error("xpose::launch: empty Kernel handle");
  }
  ...
  if (count != ki.args.size()) {
    ...
    throw Error("xpose::launch: kernel '" + ki.name + "' expects " +
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (ka.kind == ArgKind::StorageBuffer) {
      if (!la.buffer) {
        throw Error("xpose::launch('" + ki.name + "'): " + arg_label(ki, i) +
                    " is a global-memory pointer — pass an xpose::Buffer, not "
                    "a POD value");
      }
      if (!la.buffer->valid()) {
        throw Error("xpose::launch('" + ki.name + "'): " + arg_label(ki, i) +
      ...
      if (!la.pod) {
        throw Error("xpose::launch('" + ki.name + "'): " + arg_label(ki, i) +
                    " is a POD value in the kernel — an xpose::Buffer was "
                    "passed");
      }
