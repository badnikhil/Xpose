...
- A **slim C++ Vulkan runtime library** (the part we write) loads the `.spv`, maps kernel arguments to descriptor bindings via clspv's reflection, and dispatches — aiming for `vulkore::launch(kernel, grid, args...)` one-liners instead of hundreds of lines of raw Vulkan.
...
To test across multiple Vulkan devices, use the `VULKORE_DEVICE` environment variable (matches by case-insensitive substring of device name):
```bash
