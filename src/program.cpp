...
void for_each_instruction(std::span<const uint32_t> code, Fn&& fn) {
  ...
  while (i < code.size()) {
    ...
    if (word_count == 0 || i + word_count > code.size()) {
      throw Error("xpose::Program: malformed SPIR-V (bad instruction size at word " +
                  std::to_string(i) + ")");
    }
...
struct ParseScratch {
  ...
  uint32_t constant(uint32_t id, const char* what) const {
    ...
    if (it == constants.end()) {
      throw Error(std::string("xpose::Program: reflection operand '") + what +
...
void parse_reflection(std::span<const uint32_t> code, ProgramImpl& impl) {
  ...
  if (s.reflection_set_id == 0) {
    throw Error(
        "xpose::Program: no NonSemantic.ClspvReflection instruction set in "
  ...
  for_each_instruction(code, [&](uint32_t op, const uint32_t* w, uint32_t wc) {
    ...
    auto kernel_of = [&](uint32_t id) -> KernelState& {
      ...
      if (it == s.kernel_by_id.end()) {
        throw Error("xpose::Program: reflection argument references unknown "
    ...
    switch (inst) {
      case kReflKernel: {  // Kernel %fn %name [numArgs flags attributes]
        if (wc < 7) throw Error("xpose::Program: truncated Kernel reflection");
      ...
      case kReflArgumentStorageBuffer: {
        ...
        if (wc < 9) throw Error("xpose::Program: truncated ArgumentStorageBuffer");
        KernelArg a;
      ...
      case kReflArgumentPodPushConstant: {
        ...
        if (wc < 9) throw Error("xpose::Program: truncated ArgumentPodPushConstant");
        KernelArg a;
      ...
      case kReflSpecConstantWorkgroupSize: {  // xSpecId ySpecId zSpecId
        if (wc < 8) throw Error("xpose::Program: truncated SpecConstantWorkgroupSize");
      ...
      case kReflPropertyRequiredWorkgroupSize: {  // %kernel x y z
        if (wc < 9) {
          throw Error("xpose::Program: truncated PropertyRequiredWorkgroupSize");
        }
      ...
      default:
        throw Error(
            "xpose::Program: unsupported ClspvReflection instruction " +
            std::to_string(inst) + " (" + reflection_inst_name(inst) +
            ") — xpose v1 supports storage-buffer args and -pod-pushconstant "
            "PODs only (compile kernels like tests/kernels/README.md)");
    }
  });

  if (impl.kernels.empty()) {
    throw Error("xpose::Program: module contains no kernels");
  }
  ...
  for (auto& kp : impl.kernels) {
    ...
    for (uint32_t i = 0; i < ki.args.size(); ++i) {
      if (ki.args[i].ordinal != i) {
        throw Error("xpose::Program: kernel '" + ki.name +
...
void build_pipeline(KernelState& st) {
  ...
  if (ki.push_constant_size >
    ...
    throw Error("xpose::Program: kernel '" + ki.name + "' needs " +
...
const KernelState& checked(const KernelState* s) {
  if (!s) throw Error("xpose::Kernel: empty handle (default-constructed?)");
  return *s;
}
}  // namespace
...
Program::Program(Context& ctx, std::span<const uint8_t> spirv)
  ...
  if (spirv.size() < kHeaderWords * 4 || spirv.size() % 4 != 0) {
    throw Error("xpose::Program: SPIR-V blob has invalid size (" +
  ...
  if (code[0] != kSpirvMagic) {
    throw Error(code[0] == 0x03022307u
                    ? "xpose::Program: byte-swapped SPIR-V is not supported"
                    : "xpose::Program: not a SPIR-V module (bad magic)");
  }
...
Program Program::from_file(Context& ctx, const std::string& path) {
  ...
  if (!f) throw Error("xpose::Program: cannot open '" + path + "'");
  const std::streamsize size = f.tellg();
  f.seekg(0);
  std::vector<uint8_t> bytes(static_cast<size_t>(size));
  if (!f.read(reinterpret_cast<char*>(bytes.data()), size)) {
    throw Error("xpose::Program: failed reading '" + path + "'");
  }
...
Kernel Program::kernel(const std::string& name) {
  ...
  if (!st) {
    ...
    throw Error("xpose::Program: no kernel '" + name +
