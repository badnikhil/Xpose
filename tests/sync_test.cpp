...
TEST_F(SyncTest, FenceTimeoutOnUnsignaled) {
  ...
  xpose::Buffer src = ctx_->alloc<uint32_t>(n, xpose::Usage::DeviceLocal);
  xpose::Buffer dst = ctx_->alloc<uint32_t>(n, xpose::Usage::DeviceLocal);
...
TEST_F(SyncTest, CompletionHookFiresExactlyOnce) {
  ...
  {
    VkCommandBuffer cb = ctx_->begin_one_shot();
    xpose::Fence fence = ctx_->submit(cb, [&calls] { ++calls; });
    EXPECT_TRUE(fence.wait());
  ...
  {
    VkCommandBuffer cb = ctx_->begin_one_shot();
    { xpose::Fence f = ctx_->submit(cb, [&calls2] { ++calls2; }); }  // dtor drains+fires
    ctx_->free_command_buffer(cb);
  }
  EXPECT_EQ(calls2, 1);
  ...
  {
    VkCommandBuffer cb = ctx_->begin_one_shot();
    xpose::Fence f = ctx_->submit(cb, [] { throw std::runtime_error("boom"); });
...
TEST_F(SyncTest, MoveAssignOverLiveFence) {
  ...
  xpose::Fence a = ctx_->submit(cb_a);
  xpose::Fence b = ctx_->submit(cb_b);
