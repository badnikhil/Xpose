...
TEST_F(BufferTest, ForcedStagingPartialOffsetRoundtrip) {
  for (xpose::Usage usage : {xpose::Usage::DeviceLocal, xpose::Usage::HostVisible}) {
    SCOPED_TRACE(usage == xpose::Usage::DeviceLocal ? "DeviceLocal" : "HostVisible");
    const size_t n = 256;
    const auto base = pattern_u32(n, 0x33333333u);
    xpose::Buffer buf = ctx_->alloc<uint32_t>(n, usage);
...
TEST_F(BufferTest, MoveAssignOverLiveBufferReleasesOld) {
  xpose::Buffer a = ctx_->alloc<uint32_t>(32);
  xpose::Buffer b = ctx_->alloc<uint32_t>(48);
...
TEST_F(BufferTest, ZeroSizeTransferIsNoOp) {
  xpose::Buffer buf = ctx_->alloc<uint32_t>(8);
...
TEST_F(BufferTest, TypedAllocSizeForWideType) {
  const size_t n = 100;
  xpose::Buffer buf = ctx_->alloc<Vec4>(n);
...
void unaligned_case(xpose::Buffer& buf, VkDeviceSize bufsz, VkDeviceSize off,
...
TEST_F(BufferTest, BufferUnalignedSubAtomRoundtrip) {
  ...
  for (xpose::Usage usage :
       {xpose::Usage::HostVisible, xpose::Usage::DeviceLocal}) {
    for (bool force_staging : {false, true}) {
      SCOPED_TRACE(std::string("usage=") +
                   (usage == xpose::Usage::DeviceLocal ? "DeviceLocal"
                                                       : "HostVisible") +
                   " force_staging=" + (force_staging ? "yes" : "no"));
      xpose::Buffer buf = ctx_->alloc_bytes(bufsz, usage);
      buf.set_force_staging(force_staging);
      ASSERT_TRUE(buf.valid());
...
TEST_F(BufferTest, OverHeapAllocThrowsCleanly) {
  ...
  try {
    xpose::Buffer huge = ctx_->alloc<uint32_t>(count, xpose::Usage::DeviceLocal);
    ...
    huge = xpose::Buffer{};  // free immediately via move-assign over the live one
