...
class KernelFeaturesTest : public ::testing::Test {
 protected:
  ...
  static vulkore::Program& load_program(const std::string& name) {
...
std::unique_ptr<vulkore::Context> KernelFeaturesTest::ctx_;
std::vector<std::unique_ptr<vulkore::Program>> KernelFeaturesTest::programs_;
...
TEST_F(KernelFeaturesTest, Transpose2dReflection) {
  vulkore::Program& prog = load_program("transpose_2d");
  EXPECT_EQ(prog.kernel_names(), std::vector<std::string>{"transpose_2d"});

  vulkore::Kernel k = prog.kernel("transpose_2d");
...
TEST_F(KernelFeaturesTest, Grid2DEndToEnd) {
  vulkore::Kernel k = load("transpose_2d");
  ...
  for (auto [w, h] : dims) {
    ...
    vulkore::Buffer inb = ctx_->alloc<float>(in.size());
    vulkore::Buffer outb = ctx_->alloc<float>(in.size());
    inb.upload(std::span<const float>(in));

    vulkore::launch(k, {w, h}, outb, inb, w, h).wait();
...
TEST_F(KernelFeaturesTest, ReqdWorkGroupSize) {
  vulkore::Program& prog = load_program("reqd_wgsize");
  vulkore::Kernel k = prog.kernel("reqd_wgsize");
  ...
  vulkore::Buffer inb = ctx_->alloc<int32_t>(n);
  vulkore::Buffer outb = ctx_->alloc<int32_t>(n);
  inb.upload(std::span<const int32_t>(in));

  vulkore::launch(k, {n}, outb, inb, n).wait();
...
TEST_F(KernelFeaturesTest, MultiKernelModule) {
  vulkore::Program& prog = load_program("two_kernels");

  // kernel_names() returns both entry points in module order.
  EXPECT_EQ(prog.kernel_names(),
            (std::vector<std::string>{"add_one", "mul_two"}));

  vulkore::Kernel add = prog.kernel("add_one");
  vulkore::Kernel mul = prog.kernel("mul_two");
  ...
  try {
    ...
    FAIL() << "expected vulkore::Error for unknown kernel";
  ...
  vulkore::launch(add, {n}, buf, n);
  vulkore::launch(mul, {n}, buf, n);
  vulkore::launch(add, {n}, buf, n);
  vulkore::launch(mul, {n}, buf, n).wait();
...
TEST_F(KernelFeaturesTest, MixedPodPacking) {
  vulkore::Program& prog = load_program("many_pod");
  vulkore::Kernel k = prog.kernel("many_pod");
  ...
  vulkore::Buffer outb = ctx_->alloc<float>(n);

  vulkore::launch(k, {n}, outb, f, i, u, g, n).wait();
