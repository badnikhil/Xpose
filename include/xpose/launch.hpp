// xpose::launch — the headline API. CUDA-style kernel dispatch:
//
//   xpose::Fence f = xpose::launch(kernel, {n}, y_buf, x_buf, 2.0f, n);
...
//   xpose::Buffer      -> its storage-buffer descriptor (set/binding)
//   POD value          -> its slice of the push-constant block
// Wrong arity, a Buffer where a POD is expected (or vice versa), and POD size
// mismatches (e.g. a 2.0 double literal for a float arg) throw xpose::Error
...
LaunchArg make_launch_arg(const T& value) {
  ...
  if constexpr (std::is_same_v<U, Buffer>) {
    ...
    static_assert(!std::is_pointer_v<U>,
                  "xpose::launch: raw pointers cannot be kernel arguments — "
                  "pass an xpose::Buffer for global memory");
    static_assert(std::is_trivially_copyable_v<U>,
                  "xpose::launch: arguments must be xpose::Buffer or "
                  "trivially-copyable POD values");
    static_assert(sizeof(U) <= 128,
                  "xpose::launch: POD argument larger than any push-constant "
                  "block — pass it via an xpose::Buffer");
