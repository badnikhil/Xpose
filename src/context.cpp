...
Context::Context(std::optional<uint32_t> device_index) {
  ...
  if (volkGetInstanceVersion() < VK_API_VERSION_1_1) {
    throw Error("xpose: Vulkan loader does not support Vulkan 1.1");
  }

  VkApplicationInfo app{VK_STRUCTURE_TYPE_APPLICATION_INFO};
  app.pApplicationName = "xpose";
  app.pEngineName = "xpose";
  ...
  if (count == 0) throw Error("xpose: no Vulkan physical devices found");
  ...
  if (!any_compute) {
    throw Error("xpose: device '" + device_name_ + "' has no compute queue");
  }
...
uint32_t Context::pick_device(std::optional<uint32_t> explicit_index) const {
  ...
  if (explicit_index) {
    if (*explicit_index >= all_devices_.size()) {
      throw Error("xpose: device index " + std::to_string(*explicit_index) +
                  " out of range (found " +
                  std::to_string(all_devices_.size()) + " devices)");
    }
    return *explicit_index;
  }
  // 2. XPOSE_DEVICE=<substring of device name>, case-insensitive.
  if (const char* want = std::getenv("XPOSE_DEVICE"); want && *want) {
    ...
    throw Error("xpose: XPOSE_DEVICE='" + std::string(want) +
